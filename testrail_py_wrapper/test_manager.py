...
class TestManager:
    ...
    async def get_project_id_by_name(self, name: str) -> int | None:
        ...
        if name not in self.cache["projects"]:
            project_id = await self.api.get_project_id_by_name(name)
            if not project_id:
                print(f"❌ Project '{name}' not found.")
                return None
            self.cache["projects"][name] = project_id
        return self.cache["projects"][name]
    ...
    async def get_or_create_run_id(self, plan_id: int, run_name: str, suite_id: int) -> int:
        ...
        if key not in self.cache["runs"]:
            ...
            self.cache["runs"][key] = run_id
        return self.cache["runs"][key]
    ...
    async def get_or_create_section_id(self, project_id: int, suite_id: int, section_title: str) -> int:
        ...
        if key not in self.cache["sections"]:
            ...
            self.cache["sections"][key] = section_id
        return self.cache["sections"][key]
    ...
    async def get_or_create_test_id(self, run_id: int, case_title: str, project_id: int, suite_id: int,
        ...
        if key not in self.cache["tests"]:
            ...
            self.cache["tests"][key] = test_id
        return self.cache["tests"][key]
